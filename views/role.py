...
from misApp.pp import i_hasPermission
from misApp.models import tbl_employee, tbl_role, tbl_contentType,\
    tbl_permission
...
def v_role(req):
    if i_hasPermission(tbl_employee.objects.get(username=req.session.get('username')),'role','v'):
        if req.POST.get('submit_query',''):
            getparam=req.POST.get('passing_parameter','')
            roles=tbl_role.objects.filter(name__icontains=getparam)
...
def v_addRole(req):
    if i_hasPermission(tbl_employee.objects.get(username=req.session.get('username')),'role','a'):
...
def v_editRole(req,_id):
    if i_hasPermission(tbl_employee.objects.get(username=req.session.get('username')),'role','u'):
        ...
        if req.POST.get('editRole',''):
            ...
            if not name:
                errors.append("please enter role name !")
            elif name and name.lower()!=roleObj.name.lower() and tbl_role.objects.filter(name=name):
                errors.append("Role with this name already exist !")
            else:
                for i,row in enumerate(MAP):
                    MAP[i][2]=int(req.POST.get(row[1]+'_v',0))
